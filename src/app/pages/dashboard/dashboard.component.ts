...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private CandidateServiceService: CandidateService,
  ...
  loadCandidates(): void {
    this.CandidateServiceService.getAllCandidates().subscribe({
      next: (candidates) => {
        // Calculate the total number of candidates
        const totalCandidates = candidates.length;
        this.totalCandidates = totalCandidates; // Store the total candidates count

        // Update the chart data (if applicable)
        this.basicData.datasets[0].data = [this.totalCandidates];
        this.updateChart(); // Ensure the chart updates after data is loaded
      },
      error: (error) => {
        console.error('Error loading candidates:', error);
        this.totalCandidates = 0; // Reset count if there's an error
      }
    });
  }
